...
function init() {
  ...
  loadImages([
    ...
    createScreens(); // Create screen after images are loaded
    startSlideshow(); // Start the image slideshow
  });
...
function addText() {
  ...
  fontLoader.load('/fonts/Roboto.json', (font) => {
    ...
    // Third Line: "PRATEEK"
    const prateekGeometry = new TextGeometry('Prateek Dubey', {
      font: font,
      size: 6,
      depth: 0.5,
      curveSegments: 12,
...
function createScreens() {
  // Remove previous screens if they exist
  if (screenMesh1) scene.remove(screenMesh1);
  if (screenMesh2) scene.remove(screenMesh2);

  if (images.length > 0) {
    const img1 = images[currentImageIndex];
    const img2 = images[(currentImageIndex + 1) % images.length]; // Show the next image on the second screen
    const aspectRatio1 = img1.width / img1.height;
    const aspectRatio2 = img2.width / img2.height;

    // Define the size for each screen
    const width = 30;
    const height1 = width / aspectRatio1;
    const height2 = width / aspectRatio2;

    // Create the first screen
    const geometry1 = new THREE.PlaneGeometry(width, height1);
    const texture1 = new THREE.TextureLoader().load(img1.src);
    const material1 = new THREE.MeshStandardMaterial({ map: texture1 });
    screenMesh1 = new THREE.Mesh(geometry1, material1);
    screenMesh1.position.set(-63, 17, -15); // Position the first screen on the left
    screenMesh1.rotation.y = Math.PI / 180;
    scene.add(screenMesh1);

    // Create the second screen
    const geometry2 = new THREE.PlaneGeometry(width, height2);
    const texture2 = new THREE.TextureLoader().load(img2.src);
    const material2 = new THREE.MeshStandardMaterial({ map: texture2 });
    screenMesh2 = new THREE.Mesh(geometry2, material2);
    screenMesh2.position.set(63, 17, -15); // Position the second screen on the right
    screenMesh2.rotation.y = Math.PI / 180;
...
async function loadImages(imagePaths) {
  ...
  // Play background sound after all images have loaded
  playBackgroundSound();
}
...
function startSlideshow() {
  setInterval(() => {
    ...
    createScreens(); // Update the screen to show the next image
